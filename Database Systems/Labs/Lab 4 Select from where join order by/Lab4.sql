...
--3
Select Name,RollNumber,GPA
from Registration join Students on Registration.RollNumber=Students.RollNo
where GPA between 3 and 4
--4
Select Distinct Name 
from (Courses_Semester join Instructors on Courses_Semester.InstructorID=Instructors.InstructorID)
where AvailableSeats=0
--5
Select Name 
from (Courses_Semester join Instructors on Courses_Semester.InstructorID=Instructors.InstructorID) join Semester on Semester.Semester=Courses_Semester.Semester
where CourseID is null
--6
Select top 1 Students.Name,Students.RollNo
from Students join ChallanForm on Students.RollNo=ChallanForm.RollNumber
...
where Name like 'A%'
--8
Select Distinct Students.Name,Courses.CourseName
from (Courses join Registration on Registration.CourseID= Courses.CourseID) right join Students on Students.RollNo=Registration.RollNumber
--9
Select Students.Name, Registration.Semester,Courses.CreditHours
from (Students join Registration on Students.RollNo=Registration.RollNumber) join Courses on Courses.CourseID=Registration.CourseID
Order By Students.Name,Registration.Semester