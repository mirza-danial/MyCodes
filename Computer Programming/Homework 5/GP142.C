...
void dottedLine(int a_x, int a_y, int b_x, int b_y, int color)
{
	float x = a_x, y = a_y;
	int s;
	int dx = b_x - a_x, dy = b_y - a_y;
	if (abs(dx) > abs(dy))
	{
		s = abs(dx);
	}
	...
	float x_inc = dx / (float)s;
	float y_inc = dy / (float)s;
	for (int i = 0; i<s; i++)
	{
		x = x + x_inc;
		y = y + y_inc;
		if (i % 3 == 0)
		{
			GP142_pixelXY(color, round(x), round(y));
		}
	}
}
/******************************************************** bucketFill ***/
void bucket1(int x, int y, int c, int fc)
{
	if (GP142_GetColor(x, y) != c)
	{
		
		return;
	}
	else if (GP142_GetColor(x, y) == fc)
	{
		return;
	}
	else
	{
		GP142_pixelXY(fc, x, y);
		bucket1(x, y - 1, c, fc);
		bucket1(x + 1, y - 1, c, fc);
		bucket1(x - 1, y + 1, c, fc);
		}
}
void bucket2(int x, int y, int c, int fc)
{
	if (GP142_GetColor(x, y) != c)
	{
	...
	else
	{
		...
		GP142_pixelXY(fc, x, y);
		bucket2(x, y + 1, c, fc);
		bucket2(x + 1, y - 1, c, fc);
		bucket2(x - 1, y + 1, c, fc);
	}
}

void bucketFill(int x, int y, int c, int fc)
{
	bucket1(x, y, c, fc);
